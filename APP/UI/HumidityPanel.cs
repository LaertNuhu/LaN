...
public class HumidityPanel : UIPanel
{
    ...
    public override void RefreshView()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            rend1.Points[i] = new Vector2((((float)i) / (n - 1)) * width, MainInterface.Instance.data.state[i].humidity1 * height * (1f / maxValueY));
            rend2.Points[i] = new Vector2((((float)i) / (n - 1)) * width, MainInterface.Instance.data.state[i].humidity2 * height * (1f / maxValueY));
        }
    }
}
